      ...
      <div className="modal glass">
        ...
        {success ? (
          <div style={{ textAlign: 'center', padding: '20px' }}>
            ...
            <p>Your stay at {homestay.title || homestay.name} is confirmed.</p>
          </div>
        ) : step === 1 ? (
          <>
            <h2>Book your stay</h2>
            <p style={{ marginBottom: '20px', opacity: 0.7 }}>{homestay.title || homestay.name} - {homestay.location}</p>
            ...
            <form onSubmit={handleNext}>
              ...
              <div style={{ marginTop: '30px', borderTop: '1px solid var(--border-color)', paddingTop: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: '700', fontSize: '1.2rem' }}>
                  <span>Total</span>
                  <span>₹{Math.max(1, Math.ceil((new Date(booking.checkOut) - new Date(booking.checkIn)) / (1000 * 60 * 60 * 24))) * homestay.price}</span>
                </div>
              </div>
            ...
            <div style={{ marginTop: '30px', borderTop: '1px solid var(--border-color)', paddingTop: '20px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button 
                  ...
                  disabled={isProcessing}
                >
                  {isProcessing ? 'Processing...' : `Pay ₹${Math.max(1, Math.ceil((new Date(booking.checkOut) - new Date(booking.checkIn)) / (1000 * 60 * 60 * 24))) * homestay.price}`}
                </button>
              </div>
            </div>
          </>
